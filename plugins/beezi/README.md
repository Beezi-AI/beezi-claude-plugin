...
It also ships a `.mcp.json` stdio bridge (`scripts/mcp.mjs` → `lib/mcp-bridge.mjs`) that forwards MCP traffic to the Beezi server authenticated with the stored `/beezi:login` credentials — no separate MCP OAuth prompt. The bridge defaults to the production Beezi API; set `BEEZI_MCP_URL` to override.
