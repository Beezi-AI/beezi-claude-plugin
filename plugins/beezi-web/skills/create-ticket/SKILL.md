---
name: create-ticket
description: Draft and create a Beezi ticket (Task / Bug / Story). Use when the user wants to create a ticket, draft a ticket, file a bug, or add a task/story to their board (Jira, Azure DevOps) or Beezi. The drafting workflow itself is served by the Beezi MCP server.
---
...
| `list_directory`, `search_files`, `search_code`, `read_file` | Explore a connected repository to ground the draft in real code |
...
**No `beezi` tools available at all.** Either the connector isn't connected, or ticket drafting is switched off for this Beezi deployment (the server advertises an empty tool list when disabled). Ask the user to open **Settings → Connectors** and check that **Beezi** is connected. If it is connected and the tools still aren't there, drafting is disabled server-side and only their Beezi admin can turn it on.

**Authentication error.** Tell the user to reconnect: **Settings → Connectors → Beezi**, then sign in to Beezi again. The connection uses their own Beezi account via OAuth, so they must already be a Beezi user. Don't continue without a working connection.
